...
<script>
  ...
  // pré-filtrage venant d'un clic sur la carte des vignobles (?region=slug)
  function svcNorm(s){
    return (s || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '');
  }
  const regionParam = params.get('region');
  if (regionParam) {
    const slugNorm = svcNorm(regionParam);
    const matched = regions.find(r => svcNorm(r).includes(slugNorm));
    if (matched) { state.region = matched; regionSelect.value = matched; }
  }
